...
#define SET_MOV_SOURCE()                                                       \
  std::shared_ptr<scasm::scasm_operand> scasm_src = std::make_shared<scasm::scasm_operand>();                         \
...
void Codegen::gen_scasm() {
  std::shared_ptr<scasm::scasm_program> scasm_program = std::make_shared<scasm::scasm_program>();
  // std::cout << "here" << std::endl;
  for (auto func : scar.get_functions()) {
    // std::cout << "here" << std::endl;
    std::shared_ptr<scasm::scasm_function> scasm_func = std::make_shared<scasm::scasm_function>();
    ...
    for (auto inst : func->get_instructions()) {
      if (inst->get_type() == scar::instruction_type::RETURN) {
        // std::cout << "here" << std::endl;
        std::shared_ptr<scasm::scasm_instruction> scasm_inst = std::make_shared<scasm::scasm_instruction>();
        scasm_inst->set_type(scasm::instruction_type::MOV);
        SET_MOV_SOURCE();
        std::shared_ptr<scasm::scasm_operand> scasm_dst = std::make_shared<scasm::scasm_operand>();
        ...
        std::shared_ptr<scasm::scasm_instruction> scasm_inst2 = std::make_shared<scasm::scasm_instruction>();
        ...
        // std::cout << "here" << std::endl;
        std::shared_ptr<scasm::scasm_operand> scasm_dst2 = std::make_shared<scasm::scasm_operand>();
        if (inst->get_dst()->get_type() == scar::val_type::VAR) {
        ...
        if (inst->get_binop() == binop::BINOP::DIV or
          ...
          std::shared_ptr<scasm::scasm_instruction> scasm_inst3 = std::make_shared<scasm::scasm_instruction>();
          scasm_inst3->set_type(scasm::instruction_type::IDIV);
          std::shared_ptr<scasm::scasm_operand> scasm_src2 = std::make_shared<scasm::scasm_operand>();
          ...
          std::shared_ptr<scasm::scasm_instruction> scasm_inst4 = std::make_shared<scasm::scasm_instruction>();
          ...
          std::shared_ptr<scasm::scasm_operand> scasm_src3 = std::make_shared<scasm::scasm_operand>();
          ...
          std::shared_ptr<scasm::scasm_operand> scasm_src_2 = std::make_shared<scasm::scasm_operand>();
          ...
          std::shared_ptr<scasm::scasm_operand> scasm_dst3 = std::make_shared<scasm::scasm_operand>();
...
#define FIX_PSEUDO(target)                                                   \
  if (inst -> get_##target() != nullptr and inst->get_##target()->get_type() == scasm::operand_type::PSEUDO) {       \
...
void Codegen::fix_pseudo_registers() {
  ...
  for (auto &funcs : scasm->get_functions()) {
    for (auto &inst : funcs->get_instructions()) {
      // std::cout << as_integer(inst->get_type()) << std::endl;
      // if(inst->get_type() == scarlet::scasm::instruction_type::RET) {
      //   // std::cout << "saved" << std::endl;
      // }
      // std::cout << "here1" << std::endl;
      // std::cout << inst -> get_src() -> get_imm() << std::endl;
      FIX_PSEUDO(src);
...
void Codegen::fix_instructions() {
  std::shared_ptr<scasm::scasm_instruction> scasm_stack_instr = std::make_shared<scasm::scasm_instruction>();
  scasm_stack_instr->set_type(scasm::instruction_type::ALLOCATE_STACK);
  std::shared_ptr<scasm::scasm_operand> val = std::make_shared<scasm::scasm_operand>();
  val->set_type(scasm::operand_type::IMM);
  val->set_imm(stack_offset);
  scasm_stack_instr->set_src(val);

  auto shogo_inst = scasm->get_functions()[0]->get_instructions().begin();
  scasm->get_functions()[0]->get_instructions().insert(shogo_inst, std::move(scasm_stack_instr));
  std::cout << scasm->get_functions()[0]->get_instructions().size() << std::endl;
  std::cout << "BRO0" << std::endl;
  for (auto funcs : scasm->get_functions()) {
    for (auto it = funcs->get_instructions().begin();
         it != funcs->get_instructions().end(); it++) {
      std::cout << "BEGIN" << std::endl;
      if(*it == nullptr) std::cout << "NULL HGAIN SAB" << std::endl; 
      if ((*it)->get_src() != nullptr && (*it)->get_dst() != nullptr && (*it)->get_src()->get_type() == scasm::operand_type::STACK &&
        ...
        std::shared_ptr<scasm::scasm_operand> dst = std::make_shared<scasm::scasm_operand>();
...
void Codegen::codegen() {
  ...
  std::cout << "FIXING INSTRUCTIONS NOW?B" << std::endl;
  fix_instructions();
  // ###########################
